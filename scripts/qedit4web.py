...
def hello():
    return "You are running QWE ...<br/>You are being redirected to http://qpy.io/editor/?ip=" + get_lan_ip() + ":10000 on your PC browser"
...
def api_lastlog():
    ...
    try:
        ...
        if 'qpy:kivy' in target:
            return open(os.path.dirname(path) + "/last.log").read()
        elif 'qpy:webapp' or 'qpy:qpyapp' in target:
            return open(PROJ_ROOT + "/log/last.log").read()
    except:
